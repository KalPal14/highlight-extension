...
import { ENGLISH_LANGUAGE_ENTITY, URKAINIAN_LANGUAGE_ENTITY } from '../languages/stubs/languages';
import {
...
describe('WordMarksService', () => {
	...
	beforeEach(async () => {
		const module: TestingModule = await Test.createTestingModule({
			providers: [
				...
				{ provide: SourcesService, useValue: sourceServiceMock },
	...
	describe('upsert mark', () => {
		...
		beforeEach(() => {
			...
			languagesService.getOne = jest.fn().mockImplementation((languageId: number) => {
				switch (languageId) {
					...
					case URKAINIAN_LANGUAGE_ENTITY.id:
						return URKAINIAN_LANGUAGE_ENTITY;
				}
			});
			...
			wordFormsService.getOrCreateDefinition = jest
				...
				.mockImplementation(({ languageId }: DefinitionDto) => {
					return languageId === ENGLISH_LANGUAGE_ENTITY.id
						...
						: {
								...DEFINITION_WORD_FORM_URKAINIAN_ENTITY,
								examples: EXAMPLES_URKAINIAN_WORD_FORM_ENTITIES,
							};
				});
		...
		describe(`pass unmarked lemma with source`, () => {
			...
			it(`should call sourcesService to check source and upsert with lemma mark if it needs`, async () => {
				await service.upsert(dto);

				expect(sourcesService.getOrUpsert).toHaveBeenCalled();
			});
