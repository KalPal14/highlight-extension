...
import { SOURCE_ENTITY } from '../../sources/stubs/sources';
import {
...
export const UPSERT_WORD_MARK_DTO = (
	...
	return {
		...
		definitionFrom: {
			languageId: URKAINIAN_LANGUAGE_ENTITY.id,
			synonyms: SYNONYMS_ENTITIES.map(({ name }) => name),
			description: DEFINITION_WORD_FORM_URKAINIAN_ENTITY.description!,
			examples: EXAMPLES_URKAINIAN_WORD_FORM_ENTITIES.map(({ phrase }) => phrase),
		},
		definitionTo: {
